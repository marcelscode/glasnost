...
while(<IN>){
  ...
  if(1)
  {
    next if($protocol ne "BitTorrent");
...
#     }
     for(my $i=0; $i<scalar @expd; $i++){
       if(! defined $expl[$i] || !defined $expsl[$i] || ($expl[$i] == 0 && $expsl[$i] == 0)){ print STDERR "$ip $hostname expl$i\n";}
     }
  }
  ...
  for(my $n=0; $n<4; $n++)
  {
	...
	if($dir[0] == DOWNLOAD){ $base = 0; }
      ...
      if(! defined $noise1){ print STDERR "$ip $hostname $base\n"; exit;}
